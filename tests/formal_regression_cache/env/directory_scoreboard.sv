...
module directory_scoreboard (
    input  logic i_clk,
    input  logic i_rst_n,
    input  logic f_req_valid_0,
    input  logic f_req_valid_1,
    output logic f_reserve_0,
    output logic f_reserve_1,
    output logic f_slot_0_valid,
    output logic f_slot_1_valid,
    output logic f_slot_addr_equal
);
    localparam int FORMAL_SCOREBOARD_ENTRIES = 2;
...
`else
        i_cpu_req             = '{default: '0};
`endif
        i_cpu_req[0].addr     = 64'h100;
        i_cpu_req[0].id       = 8'h10;
        i_cpu_req[1].addr     = 64'h108;
        i_cpu_req[1].id       = 8'h20;
        i_cpu_req_VALID[0]    = f_req_valid_0;
        i_cpu_req_VALID[1]    = f_req_valid_1;
        i_cpu_req_READY[0]    = 1'b1;
        i_cpu_req_READY[1]    = 1'b1;
        i_cpu_rsp             = '{default: '0};
...
`endif
    ...
    directory #(
        .NUM_DIR_ENTRIES        (2),
        .NUM_SCOREBOARD_ENTRIES (FORMAL_SCOREBOARD_ENTRIES)
    ) dut (
        .i_clk                (i_clk),
        .i_rst_n              (i_rst_n),
        .i_cpu_req            (i_cpu_req),
        ...
        .o_formal_slot_0_valid   (f_slot_0_valid),
        .o_formal_slot_1_valid   (f_slot_1_valid),
        .o_formal_slot_addr_equal(f_slot_addr_equal)
    );

    assign f_reserve_0 = i_cpu_req_VALID[0] && i_cpu_req_READY[0] &&
                         o_cpu_req_allow[0];
    assign f_reserve_1 = i_cpu_req_VALID[1] && i_cpu_req_READY[1] &&
                         o_cpu_req_allow[1];

    `include "directory_env_scoreboard.sv"
`ifndef VERILATOR
    `include "directory_scoreboard_properties.sv"
`endif
endmodule
